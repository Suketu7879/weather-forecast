    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row bg-gray-800 text-white font-bold rounded-3xl p-6 m-2 items-center md:justify-between">
        <div className="text-center md:text-left">
          ...
          <div className="font-light opacity-60">
            Visibility: {data.current.vis_km || 0} KM
          </div>
          <div className="text-4xl mt-2">{data.current.temp_c || 0}° C</div>
        </div>
        <div className="flex flex-col items-center mt-4 md:mt-0">
          <img
            ...
            className="w-24"
          />
          <p className="mt-2">{data.current.condition.text || "text"}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 bg-gray-700 text-white font-bold p-6 m-2 rounded-3xl text-center">
        {[
          ...
          {
            label: "Moonrise",
            value: data.forecast.forecastday[0].astro.moonrise,
            icon: moonrise || "time",
          },
          {
            label: "Moonset",
            value: data.forecast.forecastday[0].astro.moonset,
            icon: moonset || "time",
          },
        ].map((item, index) => (
          <div key={index} className="flex flex-col items-center">
            <img
              ...
              className="w-20"
            />
            <p className="mt-2">{item.value || 0}</p>
          </div>
        ))}
      </div>
    </div>
